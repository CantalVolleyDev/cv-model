...
public class Championship {
	...
	@DAOTableRelation(
		column = @Column(name = COMPETITION_FIELD, columnDefinition = DBType2.INTEGER),
		relationColumn = Competition.IDENTIFIER_FIELD
	)
	...
	@Column(name = GENDER_FIELD, length = GENDER_FIELD_LENGTH, columnDefinition = DBType2.ENUM)
	...
	@Column(name = TYPE_FIELD, length = TYPE_FIELD_LENGTH, columnDefinition = DBType2.ENUM)
	@NotNull(message = "Le type doit être renseigné")
	private Championship.Type type;
	
	@Column(name = STATE_FIELD, length = STATE_FIELD_LENGTH, columnDefinition = DBType2.ENUM)
